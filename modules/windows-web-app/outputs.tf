...
output "app_service_id" {
  ...
  value       = azurerm_windows_web_app.app_service_windows.id
}

output "app_service_name" {
  description = "Name of the App Service"
  value       = azurerm_windows_web_app.app_service_windows.name
}

output "app_service_default_site_hostname" {
  description = "The Default Hostname associated with the App Service"
  value       = azurerm_windows_web_app.app_service_windows.default_hostname
}

output "app_service_outbound_ip_addresses" {
  description = "Outbound IP adresses of the App Service"
  value       = split(",", azurerm_windows_web_app.app_service_windows.outbound_ip_addresses)
}

output "app_service_possible_outbound_ip_addresses" {
  description = "Possible outbound IP adresses of the App Service"
  value       = split(",", azurerm_windows_web_app.app_service_windows.possible_outbound_ip_addresses)
}

output "app_service_site_credential" {
  description = "Site credential block of the App Service"
  value       = azurerm_windows_web_app.app_service_windows.site_credential
}
...
output "app_service_slot_name" {
  ...
  value       = try(azurerm_windows_web_app_slot.app_service_windows_slot[0].name, null)
}
