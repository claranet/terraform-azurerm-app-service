...
resource "null_resource" "logging_storage_activation" {
  ...
  provisioner "local-exec" {
    command = <<CMD
      sas_token=$(az storage container generate-sas --name ${var.logs_storage_account_container} --connection-string "${data.azurerm_storage_account.logs_storage_account[0].primary_connection_string}" --output tsv --permissions rwdl --expiry 2032-01-01)
      token=$(az account get-access-token --subscription ${data.azurerm_subscription.current_subscription.subscription_id} --output tsv --query '@.accessToken')
